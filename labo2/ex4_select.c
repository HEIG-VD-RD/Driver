/**
* @file ex4_select.c
* @brief Exo 4 version avec select()
...
int main()
{
	...
	*HEX3_HEX0_ptr = numbers[count];
	while (!end) {
		...
		fd_set readfds;
		// We clear the file descriptor from set
		FD_ZERO(&readfds);
		// We add the file descriptor to the set
		FD_SET(fd, &readfds);

		/* Wait for interrupt */
		int ret = select(fd + 1, &readfds, NULL, NULL, NULL);
		// FD_ISSET(fd, &readfds) returns nonzero if the file descriptor is in the set and zero if it is not
		if (ret > 0 && FD_ISSET(fd, &readfds)) {
			...
			//Useful in case of an error or a signal like SIGINT
			perror("select()");
			break;
		}
		...
		*pushbutton_edge = SET_VALUE;
		...
		// We clear the file descriptor from set and add it again
		FD_ZERO(&readfds);
		FD_SET(fd, &readfds);
		...
		//Waiting for the second interrupt
